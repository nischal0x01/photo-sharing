#!/usr/bin/env python3
"""
Secure Client Gallery — Windows build
...
Run:  python gallery_windows.py
      (or double-click start_gallery.bat)
"""
...
APP_NAME = "Secure Gallery  ·  Windows"
# Windows: store config in %LOCALAPPDATA% so it survives roaming profile issues
CONFIG_PATH = Path.home() / "AppData" / "Local" / ".secure_gallery_windows.json"
DEFAULT_TITLE = "Client Gallery"
DEFAULT_PORT = 8080
CHUNK_SIZE = 16 * 1024 * 1024  # 16 MB — handles 50 GB+ without RAM pressure
...
<div class="head">
  ...
  <div class="meta">{stats}  ·  Read-only delivery  ·  Click a file to download</div>
</div>
{body_html}
</div></body></html>"""
...
class Window(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        app_title.setFont(f)
        subtitle = QLabel("Windows  ·  Read-only share  ·  PySide6")
        ...
        self.folder_input.setPlaceholderText(r"C:\Users\You\Pictures\Delivery")
        ...
        self.log_view.setStyleSheet(
            "background:#111827;color:#d1fae5;font-family:Consolas,monospace;font-size:12px;"
        )
    ...
    def _open_folder(self) -> None:
        ...
        if not path.exists():
            ...
            return
        os.startfile(str(path))  # Windows-native open
